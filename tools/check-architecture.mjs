...
for (const file of await walk(src)) {
  ...
  for (const specifier of imports(await readFile(file, "utf8"))) { const to = destination(join(file, ".."), specifier); if (to && !rules[from]?.includes(to)) errors.push(`${relative(root, file)} imports ${to}; ${from} may depend only on ${rules[from]?.join(", ") || "nothing"}`); }
}
const fixture = await readFile(join(root, "fixtures/architecture/forbidden-domain.ts"), "utf8");
if (!imports(fixture).some((specifier) => specifier.includes("src/config"))) errors.push("forbidden fixture no longer demonstrates a domain -> config import");
if (fixtureMode) {
  const fixtureImports = imports(fixture);
  if (fixtureImports.some((specifier) => specifier.includes("src/config"))) errors.push("fixtures/architecture/forbidden-domain.ts imports config; domain may depend only on nothing");
}
if (errors.length) { console.error("Architecture violations:\n" + errors.join("\n")); process.exit(1); }
console.log("Architecture source check passed (including forbidden-import fixture).\nAllowed delivery -> application direction verified.");

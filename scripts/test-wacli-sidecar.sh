...
grep -F 'COPY container/wacli/control.mjs' "$dockerfile" >/dev/null || fail 'control bridge is not in the image'
...
for operation in control pair follow-sync health; do
  grep -F "${operation})" "$entrypoint" >/dev/null || fail "operation ${operation} is missing"
done
grep -F 'echohoard-wacli-control.mjs' "$entrypoint" >/dev/null || fail 'control bridge source is missing'
grep -F 'pair/cancel' "$root/container/wacli/control.mjs" >/dev/null || fail 'cancel endpoint is missing'
grep -F 'SIGTERM' "$root/container/wacli/control.mjs" >/dev/null || fail 'cancel does not terminate the auth process'
node --test "$root/container/wacli/control.test.mjs"
...
grep -F 'wacli-account-a:/var/lib/wacli/accounts/account-a' "$compose_file" >/dev/null || fail 'named account store is missing'
grep -F 'command: ["control"]' "$compose_file" >/dev/null || fail 'control bridge is not composed'
grep -F 'read_only: true' "$compose_file" >/dev/null || fail 'sidecars are not read-only by default'
grep -F 'restart: unless-stopped' "$compose_file" >/dev/null || fail 'control bridge restart policy is missing'

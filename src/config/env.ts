...
const schema = z.object({
  ...
  ECHOHOARD_WORKER_SQLITE_MAX_OUTPUT_BYTES: z.coerce
    ...
    .default(0),
  ECHOHOARD_WORKER_IMPORT_TRANSACTION_TIMEOUT_MS: z.coerce
    .number()
    .int()
    .min(1_000)
    .max(86_400_000)
    .default(21_600_000),
  ECHOHOARD_WORKER_IMPORT_SPOOL_MAX_BYTES: z.coerce
    .number()
    .int()
    .min(1 * 1024 * 1024)
    .max(64 * 1024 * 1024 * 1024)
...
export function parseEnv(input: NodeJS.ProcessEnv = process.env): EchohoardEnv {
  return validatedSchema.parse({
    ...
    ECHOHOARD_WORKER_IMPORT_TRANSACTION_TIMEOUT_MS:
      input.ECHOHOARD_WORKER_IMPORT_TRANSACTION_TIMEOUT_MS,
    ECHOHOARD_WORKER_IMPORT_SPOOL_MAX_BYTES: input.ECHOHOARD_WORKER_IMPORT_SPOOL_MAX_BYTES,

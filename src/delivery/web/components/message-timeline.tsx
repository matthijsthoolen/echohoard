...
export function MessageTimeline({
  ...
  return (
    <section className="timeline-panel" aria-labelledby="timeline-heading">
      ...
      <GroupingControls conversationId={conversationId} />
...
function GroupingControls({ conversationId }: { readonly conversationId: string }) {
  const [open, setOpen] = useState(false);
  const [selected, setSelected] = useState<string[]>([]);
  const [conversations, setConversations] = useState<readonly { id: string; title: string }[]>([]);
  const [status, setStatus] = useState<string>();
  const [groupingVersion, setGroupingVersion] = useState(0);
  const [mergeAuditId, setMergeAuditId] = useState<string>();
  useEffect(() => {
    if (!open) return;
    void fetch("/api/conversations?limit=50", { credentials: "same-origin" })
      .then((r) => r.json())
      .then((body: { items?: { id: string; title: string }[] }) =>
        setConversations((body.items ?? []).filter((item) => item.id !== conversationId)),
      )
      .catch(() => setStatus("Source chats could not be loaded."));
  }, [conversationId, open]);
  const submit = async (action: "merge" | "unmerge") => {
    if (action === "merge" && selected.length === 0) return;
    ...
    const response = await fetch(
      `/api/conversations/${encodeURIComponent(conversationId)}/grouping`,
      {
        ...
        body: JSON.stringify({
          ...
          expectedVersion: groupingVersion,
          ...(action === "unmerge" && mergeAuditId ? { auditId: mergeAuditId } : {}),
        }),
      },
    );
    if (response.status === 409) {
      setStatus(
        "This chat changed in another tab. Reload to keep the current grouping, then retry.",
      );
      return;
    }
    if (!response.ok) {
    ...
    const result = (await response.json()) as { version?: number; auditId?: string };
    if (typeof result.version === "number") setGroupingVersion(result.version);
    if (action === "merge" && typeof result.auditId === "string") setMergeAuditId(result.auditId);
    setStatus(
      action === "merge"
        ...
        <div className="grouping-panel">
          ...
          <div className="grouping-actions">
            ...
            <button
              className="button"
              type="button"
              disabled={!selected.length || !mergeAuditId}
